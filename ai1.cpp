...
int moveordering_evaluate_2(const board& b) {
    int res=0;
    auto it = former_transpose_table.find(b);
    if (it != former_transpose_table.end()) {
        return bonus - it->second.value;    
...
int nega_alpha(board b, int depth, bool passed, int alpha, int beta) {
    ...
    if (depth == 0) return evaluate_endgame(b);
    ...
    if (child_nodes.empty()) {
      if(passed) return evaluate_endgame(b);
        b.player = 1 - b.player;
        return -nega_alpha(b, depth, true, -beta, -alpha);
    }

    sort(child_nodes.begin()+1, child_nodes.end());
...
int search(board b, int depth) {
    int depth_end=12;
    int start = max(1, depth - 3);
    int res = -1;
    if(b.count_vacant()<depth_end){
        ...
        for (int coord = 0; coord < hw2; ++coord) {
            ...
            int score = -nega_alpha(b.move(coord), depth_end, false, -beta, -alpha);
